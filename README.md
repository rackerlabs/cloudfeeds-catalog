...
The above command essentially grabs the latest release of usage-schema and updates the `pom.xml` file.
...
Catalog output URLs are now rendered at image build time from the shared template at `docker/config/common/feedscatalog.xml.tpl` plus the selected environment overlay in `docker/config/env/<environment>/catalog.env`.

Environment-specific values are provided through GitHub Environment variables during the `test/*`, `stg/*`, and `main` workflows, so shared config no longer hardcodes environment URLs.
